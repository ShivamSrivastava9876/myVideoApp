...
const createTweet = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, tweetdetails, "Tweet posted"));
  } catch (error) {
    throw new ApiError(
      500,
      `"Getting error in creating the tweet. error is ${error}"`
    );
  }
});
...
const updateTweet = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!newContent) {
      throw new ApiError(
        400,
        "Please provide content which you want to update in previous tweet."
      );
    }
